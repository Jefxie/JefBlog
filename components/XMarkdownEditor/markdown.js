...
export default {
  ...
  methods: {
    async getFile(e) {
      ...
      const _param = (await this.handler.start)
        ? await this.handler.start()
        : {};
      if (_param.file) {
        _param.file[_param.file] = fliles;
      } else {
        _param.file = fliles;
      }
      const _res = await request(this.actions, _param);

      const _d = (await this.handler.end)
        ? await this.handler.end(_res)
        : _res;

      this.insertImage(_d.data);
    },
    init() {
    ...
    previewScroll() {
      if (this.scroll === "preview") {
        ...
        markdownContent.scrollTop = parseInt(
  ...
  watch: {
    value() {
      ...
      this.timeoutId = setTimeout(() => {
       ...
       this.$emit('getMarkdown',this.previewMarkdown) 
      }, 30);
